...
// ── Declare google on window so TypeScript doesn't complain ──────────────────
declare global {
  interface Window {
    google?: {
      accounts: {
    ...
    window.google.accounts.id.initialize({
      ...
      callback: async ({ credential }: { credential: string }) => {
        try {
          ...
          if (role) body.role = role; // pass role for registration
...
function RegisterForm({ onAlert }: { onAlert: (msg: string, type?: AlertType) => void }) {
  ...
  return (
    <form onSubmit={submit}>
      ...
      {/* ── Google button — passes selected role so new users are registered correctly ── */}
