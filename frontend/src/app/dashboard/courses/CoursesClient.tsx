...
const PROVIDER_PLATFORM_MAP: Record<string, string> = {
  udemy: 'udemy', coursera: 'coursera', 'coursera.org': 'coursera', edx: 'edx',
  'linkedin learning': 'linkedin', skillshare: 'skillshare', pluralsight: 'pluralsight',
  alison: 'alison', futurelearn: 'futurelearn',
};
...
const PLATFORM_LABELS: Record<string, string> = {
  udemy: 'Udemy', coursera: 'Coursera', edx: 'edX', linkedin: 'LinkedIn Learning',
  skillshare: 'Skillshare', pluralsight: 'Pluralsight', alison: 'Alison', futurelearn: 'FutureLearn',
};

function getPlatformKey(provider: string | undefined) {
  if (!provider) return null;
  const lower = provider.toLowerCase();
  return Object.entries(PROVIDER_PLATFORM_MAP).reduce<string | null>((match, [name, key]) => match || (lower.includes(name) ? key : null), null);
}
...
export default function CoursesClient() {
    ...
    try {
      ...
      if (certsRes.success) {
        const imported = (certsRes.data || []).map((cert: any) => {
          ...
          return {
            id: cert.id, title: cert.title, provider, category: cert.skills?.[0] || 'Platform course',
            level: 'External', enrolled: true, progress: 100, external: true,
            externalUrl: cert.credentialUrl || getCoursePlatformUrl({ provider, title: cert.title, platformKey }),
            platform: provider, platformKey, platformLabel: cert.platform || provider,
            connected: connected || false, badge: 'Imported', completedAt: cert.completedAt,
          };
        });
  ...
  async function enroll(courseId: string, title: string) {
    ...
    try {
      ...
      if (res.success) { setToast(`Enrolled in ${title}! +1 Merit Coin`); setTimeout(() => setToast(''), 3000); loadCourses(); }
      else { setToast(res.message || 'Enrollment failed'); setTimeout(() => setToast(''), 3000); }
    } catch { setToast('Enrollment failed'); setTimeout(() => setToast(''), 3000); } finally { setEnrolling(null); }
  }

  function openExternalCourse(course: any) {
    const url = getCoursePlatformUrl(course);
    if (!url) { setToast('No external course link available'); setTimeout(() => setToast(''), 3000); return; }
  ...
  function clearFilters() { setSearch(''); setFilter('all'); setCategory('All'); setLevel('All Levels'); setSort('default'); }
    ...
    let result = merged.filter(c => {
      ...
      if (search.trim()) {
        ...
        if (!c.title?.toLowerCase().includes(q) && !c.category?.toLowerCase().includes(q) &&
            !c.level?.toLowerCase().includes(q) && !c.provider?.toLowerCase().includes(q)) return false;
      }
      return true;
    });
    ...
    <SidebarLayout navItems={navItems} pageTitle="Courses">
      <div style={{ color: D.text }}>
        ...
        <div className="relative rounded-2xl p-7 mb-5 overflow-hidden"
          style={{ background: 'linear-gradient(135deg, #0A1628 0%, #0D1F3C 50%, #0A1628 100%)', border: `1px solid ${D.green}25` }}>
          <div className="absolute pointer-events-none" style={{ top: -80, left: -60, width: 320, height: 320, background: `radial-gradient(circle, ${D.green}15 0%, transparent 65%)`, borderRadius: '50%' }} />
          ...
          <div className="relative z-10">
            <div className="text-[12px] font-semibold uppercase tracking-[0.12em] mb-2" style={{ color: D.green + 'cc' }}>Learning Hub</div>
            <h1 className="font-jakarta font-bold text-[2rem] text-white leading-tight mb-1">My Courses</h1>
            <p className="text-[13px]" style={{ color: D.subtext }}>Continue learning and earn Merit Coins for every module completed.</p>
          </div>
        </div>

        {platform && (
          <div className="rounded-2xl px-4 py-3 mb-4 text-sm flex items-center gap-2"
            ...
            Showing <strong style={{ color: D.accent }}>{platformLabel}</strong> courses{category !== 'All' ? ` for ${category}` : ''}.
            <button onClick={() => setPlatform('')} className="ml-auto text-[11px] border-0 bg-transparent cursor-pointer hover:opacity-70 transition-all"
              style={{ color: D.muted }}>Clear ×</button>
          </div>
        )}
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            <i className="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-xs pointer-events-none" style={{ color: D.muted }} />
            ...
            {search && (
              <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 border-0 bg-transparent p-0 cursor-pointer hover:opacity-70 transition-all"
                style={{ color: D.muted }}><i className="fas fa-times text-xs" /></button>
            )}
          </div>
          ...
          <div className="relative">
            <select value={level} onChange={e => setLevel(e.target.value)}
              ...
              style={{ ...selectStyle, color: level !== 'All Levels' ? D.accent : D.text, borderColor: level !== 'All Levels' ? D.accent : D.border }}>
              {LEVELS.map(l => <option key={l} value={l} style={{ background: '#0D1525', color: D.text }}>{l}</option>)}
            </select>
            <i className="fas fa-chevron-down absolute right-3 top-1/2 -translate-y-1/2 text-[10px] pointer-events-none" style={{ color: D.muted }} />
          </div>

          <div className="relative">
            <select value={sort} onChange={e => setSort(e.target.value)}
              className="pl-3.5 pr-8 py-2.5 rounded-xl text-sm font-medium font-[inherit] outline-none cursor-pointer transition-all"
              style={{ ...selectStyle, color: sort !== 'default' ? D.accent : D.text, borderColor: sort !== 'default' ? D.accent : D.border }}>
              {SORT_OPTIONS.map(o => <option key={o.value} value={o.value} style={{ background: '#0D1525', color: D.text }}>{o.label}</option>)}
            </select>
            <i className="fas fa-arrow-up-down absolute right-3 top-1/2 -translate-y-1/2 text-[10px] pointer-events-none" style={{ color: D.muted }} />
          </div>
          ...
          <div className="flex gap-1 p-1 rounded-xl" style={{ background: D.input, border: `1px solid ${D.border}` }}>
            {(['all', 'enrolled', 'available'] as const).map(f => (
              <button key={f} onClick={() => setFilter(f)}
                ...
                style={{ background: filter === f ? D.accent : 'transparent', color: filter === f ? 'white' : D.muted }}>
                {f}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-64">
            <div className="w-8 h-8 rounded-full border-2 animate-spin" style={{ borderColor: D.accent + '30', borderTopColor: D.accent }} />
          </div>
        ) : (
                ...
                <div key={`${course.source || 'local'}-${course.id}`} className="rounded-2xl overflow-hidden group hover:-translate-y-1 transition-all duration-200"
                  ...
                  <div className="h-28 relative flex items-center justify-center" style={{ background: `linear-gradient(135deg, ${color}20, ${color}35)` }}>
                    ...
                    {course.enrolled && course.progress > 0 && (
                      <div className="absolute bottom-0 left-0 right-0 h-1.5" style={{ background: 'rgba(255,255,255,0.1)' }}>
                  ...
                  <div className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      {course.category && (
                        <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{ background: color + '18', color }}>{course.category}</span>
                      )}
                      {course.level && <span className="text-[11px]" style={{ color: D.muted }}>{course.level}</span>}
                    </div>

                    <h3 className="font-jakarta font-bold text-[15px] tracking-tight mb-1 leading-tight text-white">{course.title}</h3>

                    <div className="flex items-center gap-3 text-xs mb-3" style={{ color: D.subtext }}>
                      {course.provider && <span><i className="fas fa-building mr-1" style={{ color: D.muted }} />{course.provider}</span>}
                      {course.modules && <span><i className="fas fa-layer-group mr-1" style={{ color: D.muted }} />{course.modules} modules</span>}
                      {course.duration && <span><i className="fas fa-clock mr-1" style={{ color: D.muted }} />{course.duration}</span>}
                    </div>
                      ...
                      <>
                        {course.coins && (
                          <div className="flex items-center gap-1.5 mb-3">
                            ...
                            <span className="text-xs font-semibold" style={{ color: D.amber }}>Earn {course.coins} Merit Coins</span>
                          </div>
                        )}
                        <button disabled={enrolling === course.id} onClick={() => handleCourseAction(course)}
        ...
        {!loading && visible.length === 0 && (
          <div className="text-center py-16">
            <div className="w-16 h-16 rounded-2xl grid place-items-center mx-auto mb-4" style={{ background: D.green + '18' }}>
            ...
            <p className="text-[13.5px] mb-4" style={{ color: D.subtext }}>Try adjusting your filters to see more results.</p>
            {hasActiveFilters && (
              <button onClick={clearFilters}
...
export default function CoursesClient() {
  ...
  async function openMeritlivesCourse(course: any) {
    if (!course.slug && !course.url) {
      setToast('Course link unavailable'); setTimeout(() => setToast(''), 3000); return;
    }
    ...
    if (!slug) {
      window.open(course.url, '_blank', 'noopener,noreferrer'); return;
    }
    try {
