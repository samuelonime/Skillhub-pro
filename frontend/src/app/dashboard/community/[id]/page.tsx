/**
 * Public community share page — /community/[id]
 *
 ...
 *   1. Sharing a post now pulls the post image + title + body (OG tags below).
...
/* ── Visible public page ────────────────────────────────────────────────── */
export default async function PublicCommunityPostPage(
  { params }: { params: Promise<{ id: string }> },
) {
  const { id } = await params;
  const post = await getPost(id);
