...
function getTier(c: number) {
  return c >= 5000 ? 'platinum' : c >= 2000 ? 'gold' : c >= 500 ? 'silver' : 'bronze';
}
function MeritBadge({ coins }: { coins: number }) {
  const t = TIERS[getTier(coins)];
  return (
    <span className="inline-flex items-center gap-1 text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: t.bg, color: t.color }}>
      {t.icon} {t.label} · {coins.toLocaleString()}
    </span>
  );
}
function TierPill({ tier }: { tier: string }) {
  const t = TIERS[tier]; if (!t) return <span className="text-[11px] text-[#9898b8]">Any</span>;
  return <span className="inline-flex items-center gap-0.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: t.bg, color: t.color }}>{t.icon} {t.label}+</span>;
}
...
function StatusSelect({ appId, current, onChange }: any) {
  ...
  return (
    <select value={current} disabled={loading} onChange={e => update(e.target.value)}
      className="text-[11px] font-semibold px-2.5 py-1 rounded-full border-0 cursor-pointer outline-none appearance-none disabled:opacity-50"
      style={{ background: sbg, color: sc }}>
      {['reviewing','shortlisted','interviewing','hired','rejected'].map(o => (
    ...
    <div className="fixed inset-0 z-50 flex" onClick={onClose}>
      ...
      <div className="w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-[#e8e8f0] sticky top-0 bg-white z-10">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl bg-[#f5f5fb] border-0 cursor-pointer text-[#6b6b8a] hover:bg-[#f0f0f8] grid place-items-center"><i className="fas fa-times text-xs" /></button>
        </div>
        {loading ? (
          <div className="p-5 flex flex-col gap-3"><Sk h="h-20" r="rounded-2xl" />{[1,2,3,4].map(i => <Sk key={i} h="h-12" r="rounded-xl" />)}</div>
        ) : profile ? (
          <div className="p-5">
            ...
            <div className="grid grid-cols-3 gap-2 mb-5">
              {[
                ...
                { label:'Certs',  value:profile.certificates?.length||0, color:'#22c55e', bg:'#f0fdf4' },
              ...
              <div className="mb-5">
                ...
                {profile.certificates.map((c: any) => (
                  <div key={c.id} className="flex items-center gap-2.5 py-2.5 border-b border-[#f0f0f8] last:border-0">
                    ...
                    {c.credentialUrl && <a href={c.credentialUrl} target="_blank" rel="noreferrer" className="text-[11px] text-[#5b4cf5] font-semibold no-underline">View ↗</a>}
                  </div>
                ))}
              </div>
            )}
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {profile.platforms.map((p: string) => (
                    <span key={p} className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-[#f0fdf4] text-[#22c55e]"><i className="fas fa-check-circle mr-1" />{p}</span>
                  ))}
                </div>
              </div>
            )}
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-xl shadow-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-col gap-3">
          {[
            ...
            { label:'Location *', key:'location', placeholder:'e.g. Lagos, Nigeria or Remote' },
            { label:'Salary / Pay Range', key:'salary', placeholder:'e.g. ₦400k–₦600k/mo or $3,000–$5,000/mo' },
            ...
            <div key={f.key}>
              ...
              <input value={(form as any)[f.key]} onChange={e => set(f.key, e.target.value)} placeholder={f.placeholder}
                className="w-full px-3.5 py-2.5 border border-[#e8e8f0] rounded-xl text-sm font-[inherit] outline-none focus:border-[#5b4cf5] focus:shadow-[0_0_0_3px_rgba(91,76,245,0.1)] transition-all" />
            </div>
          ))}
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select value={form.type} onChange={e => set('type', e.target.value)}
                className="w-full px-3.5 py-2.5 border border-[#e8e8f0] rounded-xl text-sm font-[inherit] outline-none focus:border-[#5b4cf5] transition-all bg-white cursor-pointer">
                {['Full-time','Part-time','Contract','Remote','Internship'].map(t => <option key={t}>{t}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-semibold text-[#6b6b8a] mb-1">Minimum Merit Tier</label>
              <select value={form.minTier} onChange={e => set('minTier', e.target.value)}
                className="w-full px-3.5 py-2.5 border border-[#e8e8f0] rounded-xl text-sm font-[inherit] outline-none focus:border-[#5b4cf5] transition-all bg-white cursor-pointer">
                <option value="">Any tier (visible to all)</option>
                <option value="bronze">🥉 Bronze+ (500+ coins)</option>
                <option value="silver">🥈 Silver+ (500+ coins)</option>
                <option value="gold">🥇 Gold+ (2,000+ coins)</option>
                <option value="platinum">💎 Platinum only (5,000+ coins)</option>
              </select>
            </div>
          </div>
          <div>
            ...
            <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={5}
              ...
              className="w-full px-3.5 py-2.5 border border-[#e8e8f0] rounded-xl text-sm font-[inherit] outline-none focus:border-[#5b4cf5] focus:shadow-[0_0_0_3px_rgba(91,76,245,0.1)] transition-all resize-none" />
          </div>
          <label className="flex items-center gap-3 p-3 rounded-xl bg-[#f5f5fb] cursor-pointer">
            ...
            <div>
              <div className="text-sm font-semibold text-[#0a0a0f]">Mark as Premium / Featured Job</div>
              <div className="text-[11px] text-[#6b6b8a]">Premium jobs are shown more prominently to Gold & Platinum students</div>
            </div>
          </label>
        </div>
...
export default function EmployerDashboardPage() {
  ...
  const loadOverview = useCallback(async () => {
    try { const r = await apiFetch('/employer/dashboard'); if (r.success) setOverview(r.data); } catch {}
  }, []);

  const loadJobs = useCallback(async () => {
    try {
      const p = new URLSearchParams();
      if (jobStatusFilter) p.set('status', jobStatusFilter);
  ...
  const loadTalent = useCallback(async () => {
    try {
      const p = new URLSearchParams();
      if (talentTier !== 'all') p.set('tier', talentTier);
      if (talentSearch) p.set('search', talentSearch);
      const r = await apiFetch(`/employer/talent?${p}`);
      if (r.success) { setTalent(r.data.users); setTalentTotal(r.data.total); } else setTalent([]);
    } catch { setTalent([]); }
  ...
  return (
    <SidebarLayout navItems={navItems} pageTitle="Employer Dashboard">
      ...
      {showPostJob && <PostJobModal onClose={() => setShowPostJob(false)} onPosted={() => { showToast('Job posted!'); loadJobs(); loadOverview(); }} />}

      {/* Header */}
      <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div>
          <h1 className="font-syne font-bold text-[21px] tracking-tight mb-0.5">Employer Dashboard</h1>
          <p className="text-[13px] text-[#6b6b8a]">All data live from your hiring pipeline.</p>
        </div>
      ...
      <div className="flex gap-1 bg-[#f5f5fb] p-1 rounded-xl w-fit mb-5 border border-[#e8e8f0] flex-wrap">
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-2 rounded-[9px] text-sm font-medium font-[inherit] cursor-pointer transition-all border-0 whitespace-nowrap ${tab === t.key ? 'bg-white text-[#0a0a0f] font-semibold shadow-[0_1px_5px_rgba(0,0,0,0.09)]' : 'bg-transparent text-[#6b6b8a]'}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4 max-md:grid-cols-1">
            ...
            <div className="bg-white rounded-2xl p-5 border border-[#e8e8f0]">
              ...
              <p className="text-xs text-[#6b6b8a] mb-4">Distribution of your applicants by tier.</p>
          ...
          <div className="bg-white rounded-2xl p-5 border border-[#e8e8f0]">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setTab('applicants')} className="text-xs font-semibold text-[#5b4cf5] bg-[#f5f5fb] px-3 py-1.5 rounded-lg cursor-pointer hover:bg-[#f4f2ff] transition-all border-0">View all</button>
            </div>
            {!overview ? <div className="flex flex-col gap-3">{[1,2,3,4,5].map(i=><Sk key={i} h="h-14" r="rounded-xl"/>)}</div>
             : recent.length === 0 ? <p className="text-sm text-[#9898b8] py-4 text-center">No applicants yet. <button className="text-[#5b4cf5] font-semibold" onClick={() => setShowPostJob(true)}>Post your first job →</button></p>
             : <div className="overflow-x-auto"><table className="w-full text-sm border-collapse">
                <thead><tr>{['Candidate','Applied For','Merit Tier','Certs','Projects','Status','Action'].map(h=>(
                ...
                <tbody>{recent.map((a: any) => (
                  <tr key={a.applicationId} className="hover:bg-[#fafafd] transition-colors">
                    ...
                    <td className="py-3 px-4 border-b border-[#f0f0f8]">
                      <button onClick={() => setSelectedUser(a.id)} className="text-[12px] font-semibold text-[#5b4cf5] bg-[#f4f2ff] px-2.5 py-1 rounded-lg border-0 cursor-pointer hover:bg-[#5b4cf5] hover:text-white transition-all">Profile</button>
                    </td>
                  </tr>
                ))}</tbody>
              </table></div>}
        ...
        <div className="bg-white rounded-2xl p-5 border border-[#e8e8f0]">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button onClick={() => setShowPostJob(true)}
                className="inline-flex items-center gap-2 px-3.5 py-2 bg-[#5b4cf5] text-white text-sm font-semibold rounded-xl border-0 cursor-pointer hover:bg-[#7c6ff7] transition-all">
          ...
          {jobs === null ? <div className="flex flex-col gap-3">{[1,2,3,4].map(i=><Sk key={i} h="h-16" r="rounded-xl"/>)}</div>
           : jobs.length === 0 ? (
            <div className="py-12 text-center">
              ...
              <button onClick={() => setShowPostJob(true)} className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#5b4cf5] text-white rounded-xl text-sm font-semibold border-0 cursor-pointer hover:bg-[#7c6ff7] transition-all">
                Post your first job
              </button>
            </div>
          ) : <div className="overflow-x-auto"><table className="w-full text-sm border-collapse">
            <thead><tr>{['Job Title','Type','Location','Min. Tier','Applicants','Status','Posted','Actions'].map(h=>(
              <th key={h} className="py-2.5 px-4 text-left text-[11px] font-semibold text-[#6b6b8a] border-b border-[#e8e8f0] uppercase tracking-wide whitespace-nowrap">{h}</th>
            ))}</tr></thead>
              ...
              return (
                <tr key={job.id} className="hover:bg-[#fafafd] transition-colors">
                  <td className="py-3.5 px-4 border-b border-[#f0f0f8]">
                    ...
                    {job.isPremium && <span className="text-[10px] font-bold text-white px-1.5 py-0.5 rounded-full bg-[#5b4cf5]">Featured</span>}
                  </td>
                  ...
                  <td className="py-3.5 px-4 border-b border-[#f0f0f8]"><TierPill tier={job.minTier}/></td>
                  ...
                  <td className="py-3.5 px-4 border-b border-[#f0f0f8]">
                    <button onClick={() => setShowPostJob(true)} className="text-[12px] font-semibold text-[#5b4cf5] bg-[#f4f2ff] px-2.5 py-1 rounded-lg border-0 cursor-pointer hover:bg-[#5b4cf5] hover:text-white transition-all">Edit</button>
                  </td>
                </tr>
              );
            })}</tbody>
      ...
      {tab === 'applicants' && (
        <div className="bg-white rounded-2xl p-5 border border-[#e8e8f0]">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            <span className="font-syne font-bold text-[15px]">All Applicants <span className="text-[#9898b8] font-normal text-sm">(sorted by Merit Coins)</span></span>
            <div className="flex gap-2 flex-wrap">
              <div className="flex gap-1 bg-[#f5f5fb] p-1 rounded-xl border border-[#e8e8f0]">
                {(['all','platinum','gold','silver','bronze'] as const).map(t => (
                  <button key={t} onClick={() => setTierFilter(t)}
                    className={`px-2.5 py-1.5 rounded-[8px] text-[11px] font-semibold border-0 cursor-pointer transition-all ${tierFilter===t?'bg-white shadow-[0_1px_4px_rgba(0,0,0,0.08)] text-[#0a0a0f]':'bg-transparent text-[#6b6b8a]'}`}>
          ...
          {applicants===null ? <div className="flex flex-col gap-3">{[1,2,3,4,5].map(i=><Sk key={i} h="h-16" r="rounded-xl"/>)}</div>
           ...
           : <div className="overflow-x-auto"><table className="w-full text-sm border-collapse">
            <thead><tr>{['#','Candidate','Applied For','Merit Tier','Skills','Certs','Projects','Status','Action'].map(h=>(
            ...
            <tbody>{applicants.map((a: any, idx: number) => (
              <tr key={a.applicationId} className={`hover:bg-[#fafafd] transition-colors ${a.meritCoins>=5000?'bg-[#fafaff]':''}`}>
                <td className="py-3 px-3 border-b border-[#f0f0f8]">
                  <span className={`w-6 h-6 rounded-full grid place-items-center text-[11px] font-bold ${idx===0?'bg-[#f59e0b] text-white':idx===1?'bg-[#9898b8] text-white':idx===2?'bg-[#cd7f32] text-white':'bg-[#f5f5fb] text-[#6b6b8a]'}`}>{idx+1}</span>
                </td>
                <td className="py-3 px-3 border-b border-[#f0f0f8]">
                  <div className="flex items-center gap-2.5">
                    <Avatar name={a.name} avatar={a.avatar}/>
                    <div><div className="font-semibold text-[13px]">{a.name}</div>{a.title&&<div className="text-[10px] text-[#9898b8]">{a.title}</div>}</div>
                  </div>
                </td>
        ...
        <>
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            <div>
              ...
              <p className="text-[12px] text-[#6b6b8a]">{talentTotal.toLocaleString()} verified candidates.</p>
            </div>
            <div className="flex gap-2 flex-wrap">
              <input value={talentSearch} onChange={e => setTalentSearch(e.target.value)}
                onKeyDown={e => e.key==='Enter'&&loadTalent()}
              ...
              <div className="flex gap-1 bg-[#f5f5fb] p-1 rounded-xl border border-[#e8e8f0]">
                {(['all','platinum','gold','silver','bronze'] as const).map(t=>(
                  <button key={t} onClick={()=>setTalentTier(t)}
                    className={`px-2.5 py-1.5 rounded-[8px] text-[11px] font-semibold border-0 cursor-pointer transition-all ${talentTier===t?'bg-white shadow-[0_1px_4px_rgba(0,0,0,0.08)] text-[#0a0a0f]':'bg-transparent text-[#6b6b8a]'}`}>
                    {t==='all'?'All':`${TIERS[t].icon} ${TIERS[t].label}`}
                  </button>
                ))}
              </div>
                  ...
                  <div key={u.id} className="bg-white rounded-2xl p-5 border-2 border-[#7c3aed]/25 relative overflow-hidden hover:-translate-y-0.5 hover:shadow-[0_8px_24px_rgba(124,58,237,0.12)] transition-all">
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-4">
                      {[{v:u.skills?.length||0,l:'Skills',c:'#5b4cf5',bg:'#f4f2ff'},{v:u.certCount,l:'Certs',c:'#22c55e',bg:'#f0fdf4'},{v:u.projectCount,l:'Projects',c:'#f59e0b',bg:'#fffbeb'}].map(s=>(
                        <div key={s.l} className="text-center rounded-xl p-2" style={{background:s.bg}}>
                          <div className="font-syne font-bold text-sm" style={{color:s.c}}>{s.v}</div>
                          <div className="text-[9px] text-[#9898b8]">{s.l}</div>
                        </div>
                      ))}
                    </div>
                    <button onClick={()=>setSelectedUser(u.id)} className="w-full py-2 text-xs font-semibold text-white bg-[#5b4cf5] rounded-xl border-0 cursor-pointer hover:bg-[#7c6ff7] transition-all">View Profile</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl p-5 border border-[#e8e8f0]">
            {talent===null?<div className="flex flex-col gap-3">{[1,2,3,4,5].map(i=><Sk key={i} h="h-14" r="rounded-xl"/>)}</div>
             ...
             :<div className="overflow-x-auto"><table className="w-full text-sm border-collapse">
              <thead><tr>{['#','Candidate','Merit Tier','Skills','Certs','Projects','Platforms','Action'].map(h=>(
                <th key={h} className="py-2.5 px-3 text-left text-[11px] font-semibold text-[#6b6b8a] border-b border-[#e8e8f0] uppercase tracking-wide whitespace-nowrap">{h}</th>
              ))}</tr></thead>
                ...
                <tr key={u.id} className={`hover:bg-[#fafafd] transition-colors ${u.meritCoins>=5000?'bg-[#fafaff]':''}`}>
                  ...
                  <td className="py-3 px-3 border-b border-[#f0f0f8]">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div><div className="font-semibold text-[13px]">{u.name}</div>{u.title&&<div className="text-[10px] text-[#9898b8]">{u.title}</div>}</div>
                    </div>
                  </td>
                  ...
                  <td className="py-3 px-3 border-b border-[#f0f0f8]">
                    {u.platforms?.length>0?<span className="text-[11px] text-[#5b4cf5]">{u.platforms.slice(0,2).join(', ')}{u.platforms.length>2?` +${u.platforms.length-2}`:''}</span>:<span className="text-[11px] text-[#9898b8]">—</span>}
                  </td>
                  <td className="py-3 px-3 border-b border-[#f0f0f8]">
                    <button onClick={()=>setSelectedUser(u.id)} className="text-[12px] font-semibold text-[#5b4cf5] bg-[#f4f2ff] px-2.5 py-1 rounded-lg border-0 cursor-pointer hover:bg-[#5b4cf5] hover:text-white transition-all">Profile</button>
                  </td>
                </tr>
              ))}</tbody>
            </table></div>}
