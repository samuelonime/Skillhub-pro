...
export function SharedPostsView({ userId, isLoggedIn }: {
  userId: string;
  isLoggedIn?: boolean;
}) {
  ...
  async function loadShares() {
    try {
      setLoading(true);
      const res = await apiFetch(`/community/user/${userId}/shared?page=${page}`);
      if (res.success) {
        if (page === 1) {
      ...
      console.error('Failed to load shared posts:', e);
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="space-y-4">
        {shares.map((share) => (
          <div key={share.id}>
            ...
            <PostCard post={share.post} isLoggedIn={isLoggedIn} />
          </div>
        ))}
      </div>
