...
router.get('/:jobId', authenticate, async (req, res) => {
  try {
    // Job.skills is String[] — plain list of skill name tags
    ...
    // User.skills is now a UserSkill relation (name, level, verified)
    // User.enrolledPaths is SkillPathEnrollment via the named relation
    // User.resume is the ResumeUpload relation
    ...
    // Normalise job skills (String[]) to lowercase
    const requiredSkills = (job.skills || []).map(s => s.trim().toLowerCase()).filter(Boolean);
...
// ── GET /api/v1/skill-gap/overview/market — general skill gap vs market ───────
router.get('/overview/market', authenticate, async (req, res) => {
  try {
    // Job.skills is String[] — aggregate top demanded skill names across active jobs
    ...
    // User.skills is now a UserSkill relation
...
function getCourseSuggestions(skill) {
  const map = {
    ...
    'django':          [{ platform: 'Udemy',        title: 'Python and Django Full Stack Developer',   url: 'https://udemy.com/course/python-and-django-full-stack-web-developer-bootcamp/' }],
  };
